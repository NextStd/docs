# Summary

- [Introduction](README.md)
  - [Why NextStd?](intro/why-nextstd.md)
  - [Safety Guarantees](intro/safety.md)

- [Getting Started](getting-started/getting-started.md)
  - [Installation & Linking](getting-started/installation.md)
  - [Your First Program](getting-started/hello-world.md)
  - [Build System Integration](getting-started/build-systems.md)

- [Error Handling (`ns_error`)](error/README.md)
  - [The Try/Except Macros](error/try-except.md)
  - [Null Pointer Protection](error/null-safety.md)
  - [Error Codes Reference](error/codes.md)

- [Input & Output (`ns_io`)](io/README.md)
  - [Safe Printing (`ns_print`)](io/printing.md)
  - [Safe Reading (`ns_read`)](io/reading.md)
  - [Terminal Colors](io/colors.md)

- [Strings (`ns_string`)](string/README.md)
  - [Small String Optimization (SSO)](string/sso-architecture.md)
  - [Creating & Freeing Strings](string/lifecycle.md)
  - [String Concatenation](string/concatenation.md)

- [Data Structures (`ns_data`)](data_structures/README.md)
  - [Dynamic Arrays (`ns_vec`)](data_structures/vectors.md)
  - [Key-Value Maps (`ns_hashmap`)](data_structures/hashmap.md)

- [Examples](examples/README.md)
  - [Safe User Input](examples/safe-input.md)
  - [Building a CLI Menu](examples/cli-menu.md)
  - [String Manipulation](examples/string-manipulation.md)

- [Contributing](contributing/README.md)
  - [Architecture Overview](contributing/architecture.md)
  - [Adding New Modules](contributing/new-modules.md)
